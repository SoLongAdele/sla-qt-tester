        ...
        if api_key and AI_LIB_AVAILABLE:
            try:
                self.ai_client = OpenAI(
                    api_key=api_key,
                    base_url=api_base_url or "https://spark-api-open.xf-yun.com/v1"
                )
                logger.info(f"AI 客户端初始化成功 (Base URL: {api_base_url or '讯飞星火'})")
